...
setwd("D:/github/Bayesian-lumber-strength/R/newmodel")
require(rstan)
require(MASS)
require(loo)
nsims <- 100
...
for(isim in 1:nsims){
  ...
  ## model 1 
  dmgth_mod <- stan_model("newmodel_thresh.stan")
  options(mc.cores = parallel::detectCores())
  
  init_dmg <- function() {
    list(mu = c(35,8), sigma = c(10,1), rho = .5, alpha = rep(1,6),thresh = .7 )
  }
  ...
  realth_fit <- sampling(object = dmgth_mod,
                         data = data_stan,init = init_dmg)
  # print(realth_fit, par = c('mu', 'sigma','rho','alpha','thresh'))
  
  a <- summary(realth_fit, par = c('mu', 'sigma','rho','alpha','thresh'))$summary[,1]
  ## model 2 
  dmgtest_mod <- stan_model("newmodel_test.stan")
  options(mc.cores = parallel::detectCores())
