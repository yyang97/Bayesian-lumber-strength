...
functions {
  ...
  real DmgR_lpdf(real[] x, real l,real[] mu, real[] sigma,real rho, real alpha) {
    ...
    real derivative;
    if(x[3] == 1){
      //woodloglik = (-(x[1] - mu[1])^2/(2*sigma[1]^2))-log(sigma[1]);
      woodloglik = normal_lpdf(x[1]|mu[1],sigma[1]);
    }
    else{
      ...
      woodloglik = normal_lpdf(x_real|mu[2],sigma[2]) + normal_lccdf(a_l | 0, 1) + log(derivative);
    }
    return woodloglik;
  }
  ...
  real DmgT_lpdf(real[] x, real l,real[] mu, real[] sigma,real rho, real alpha) {
    ...
    real derivative;
    if(x[3] == 1){
      woodloglik = normal_lpdf(x[1]|mu[2],sigma[2]);
    }
    else{
      x_real = (x[2] + sqrt(x[2]^2 + 4*alpha))*0.5;
      derivative = 0.5+x[2]/(2*sqrt(x[2]^2 + 4* alpha));
      a_l = (l - mu[2]-rho*(sigma[2]/sigma[1])*(x_real-mu[1]))/(sigma[2]*sqrt(1-rho^2));
      woodloglik = normal_lpdf(x_real|mu[1],sigma[1]) + normal_lccdf(a_l | 0, 1) + log(derivative);
    }
    return woodloglik;
  }
...
// vector[N_R20+N_R40+N_R60+N_T20+N_T40+N_T60+N_x+N_y] log_lik;
  vector[N_R20] log_lik_R20;
  vector[N_R40] log_lik_R40;
  vector[N_R60] log_lik_R60;
  vector[N_T20] log_lik_T20;
  vector[N_T40] log_lik_T40;
  vector[N_T60] log_lik_T60;
  vector[N_x] log_lik_R100;
  vector[N_y] log_lik_T100;
    for (n in 1:N_R20){
    log_lik_R20[n] = DmgR_lpdf(X_R20[n,]|l_R20,mu,sigma,rho,alpha_R20);
   //log_lik_R20[n] = spldR_lpdf(X_R20[n,]|l_R20,mu,sigma,rho);
  }
    for (n in 1:N_R40){
    log_lik_R40[n] = DmgR_lpdf(X_R40[n,]|l_R40,mu,sigma,rho,alpha_R40);
  }
    for (n in 1:N_R60){
    log_lik_R60[n] = DmgR_lpdf(X_R60[n,]|l_R60,mu,sigma,rho,alpha_R60);
    //log_lik_R60[n] = spldR_lpdf(X_R60[n,]|l_R60,mu,sigma,rho);
  }
    for (n in 1:N_T20){
    log_lik_T20[n] = DmgT_lpdf(X_T20[n,]|l_T20,mu,sigma,rho,alpha_T20);
  }  
    for (n in 1:N_T40){
    log_lik_T40[n] = DmgT_lpdf(X_T40[n,]|l_T40,mu,sigma,rho,alpha_T40);
  }    
    for (n in 1:N_T60){
    log_lik_T60[n] = DmgT_lpdf(X_T60[n,]|l_T60,mu,sigma,rho,alpha_T60);
  }    
    for (n in 1:N_x){
    log_lik_R100[n] =  normal_lpdf(t_x[n]|mu[1],sigma[1]);
  } 
    for (n in 1:N_y){
    log_lik_T100[n] =  normal_lpdf(t_y[n]|mu[2],sigma[2]);
  }   
}
