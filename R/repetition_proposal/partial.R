library(ggplot2)
library(ggpubr)
partial <- readRDS("sim_partial.rds")

mean_df <- as.data.frame(partial$mean)


# mean 
colMeans(partial$mean)
...
theta_true <- c(45,5.5,13,1,.7,.7,1,.9,.7,1,.9,.7)

upper <- partial$upper
lower <- partial$lower

coverage <- matrix(NA, ncol = 12, nrow = 1000)
for (icov in 1:1000){
